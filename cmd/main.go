...
func circlenoise() {
	...
	for i := range gena.Range(_iters) {
		...
		for j := range angles {
			angles[j] = noise.NoiseV2(complex(float64(j), 0)+gena.Polar(1, ralpha)) + ralpha
		}

		peepo := [4096]float64{}
		for j := range peepo {
			peepo[j] = noise.NoiseV2(complex(float64(j), 1) + gena.Polar(1, ralpha))
		}
		noise2 := gena.NewPerlinNoise(peepo)

		c := newCanvas(500, 500)
		gena.FillBackground(c, gena.White)
		gena.CircleNoise(c, 0.3, 80, angles, 60, 80, 400, noise2)
...
func colorcircle2() {
	c := newCanvas(800, 800)
	gena.FillBackground(c, gena.White)
	gena.ColorCircle2(c, []color.RGBA{
		{0x11, 0x60, 0xC6, 0xFF},
		{0xFD, 0xD9, 0x00, 0xFF},
		{0xF5, 0xB4, 0xF8, 0xFF},
		{0xEF, 0x13, 0x55, 0xFF},
...
func domainwrap() {
	c := newCanvas(500, 500)
	gena.FillBackground(c, gena.Black)
	gena.DomainWarp(c, 0.01, 4, complex(4, 20), cmap, 0, 0, "")
...
func domainwrapFrames() {
	c := newCanvas(500, 500)
	gena.FillBackground(c, gena.Black)
	gena.DomainWarp(c, 0.01, 4, complex(4, 20), cmap, 0.005, 100, "./temp")
}
