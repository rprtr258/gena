...
func (cg *circleGrid) draw(ctex *Context, c Canvas, v V2, r float64) {
	rnd := rand.Intn(4)
...
func CircleGrid(c Canvas, circleNumMin, circleNumMax int) {
	cg := &circleGrid{
	...
	for i := 0; i < seg; i++ {
		for j := 0; j < seg; j++ {
			...
			ctex.Fill()
			cg.draw(ctex, c, v, w/2*RandomFloat64(0.6, 0.95))
		}
	}
}
